...
import { garminMileSymbol, garminTurnSymbol, garminWaypointSymbol, turnComment, uniqueTurnExportNames } from './garmin';
...
`;
  ...
  const reserved = [
    ...mileMarkers.map((mm) => mm.customLabel || mm.label),
    ...waypoints.filter((w) => w.enabled).map((w) => w.name),
  ];
  const turnNames = uniqueTurnExportNames(detectedTurns, routePoints, settings?.mileUnit ?? 'miles', reserved);
