    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center', maxWidth: 480, padding: '0 24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 300, margin: '0 auto' }}>
          {/* OAuth buttons */}
          <button className="btn btn-google btn-lg btn-block" onClick={() => signInOAuth('google')}>
          ...
          <button className="btn btn-apple btn-lg btn-block" onClick={() => signInOAuth('apple')}>
          ...
          {message && (
            <p style={{ color: 'var(--green)', fontSize: '0.82rem', margin: 0 }}>{message}</p>
          )}

          <button
            onClick={() => { setIsSignUp((s) => !s); setError(null); setMessage(null); }}
            style={{
              background: 'none', border: 'none', color: 'var(--text-dim)',
              cursor: 'pointer', fontSize: '0.82rem', padding: 4,
            }}
          >
